    ...
    <nav>
      <div className="top-header relative flex items-center justify-center text-xs sm:text-sm bg-black h-12 text-white">
        ...
        <div className="change-language hidden mlg:flex items-center gap-2 absolute right-24">
          <span>English</span>
          <img src="/open-menu.png" alt="Dropdown Menu" />
        </div>
      </div>
      ...
      <div className="header border-b border-gray-300 h-24 pt-6 px-5 sm:px-0 flex items-center justify-between sm:justify-evenly">
        ...
        <div className="settings flex items-center gap-5">
          <div className="search relative">
            ...
            <img className='mlg:absolute right-2 bottom-2' src="/search.png" alt="search" />
          </div>
          <div className="nav-buttons flex items-center gap-3">
            <img className='hidden mlg:block cursor-pointer' src="/Wishlist.png" alt="wishlist" />
            <img className='cursor-pointer' src="/Cart.png" alt="cart" />
            <div className="account relative text-white">
              <img onClick={() => {setaccountOptions(!accountOptions)}} className={`cursor-pointer rounded-full p-1 transition-all duration-300 ${(accountOptions) && 'bg-red-400'}`} src="/user.png" alt="user" />
              <ul className={`account-options ${(accountOptions)? 'block':'hidden'} w-60 h-52 pl-4 rounded-md absolute top-10 right-0 bg-black bg-opacity-40 backdrop-blur-md`}>
                <li onClick={() => {router.push(`/${(session) && session.user.first_name}`)}} className='cursor-pointer flex items-center gap-3 my-4'><img className="w-5" src="/account.png" alt="Account" /><p>Manage My Account</p></li>
                <li className='cursor-pointer flex items-center gap-3 my-4'><img className="w-5" src="/orders.png" alt="Orders" /><p>My Order</p></li>
                <li className='cursor-pointer flex items-center gap-3 my-4'><img className="w-5" src="/cancel.png" alt="Cancellations" /><p>My Cancellations</p></li>
                <li className='cursor-pointer flex items-center gap-3 my-4'><img className="w-5" src="/favourites.png" alt="Reviews" /><p>My Reviews</p></li>
                <li onClick={() => {signOut()}} className='cursor-pointer flex items-center gap-3 my-4'><img className="w-5" src="/logout.png" alt="Logout" /><p>Logout</p></li>
              </ul>
            </div>
          </div>
        </div>
